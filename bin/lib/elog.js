 ...
 */
function elog(msg, ...dispVars) {
    /**
     * If DEBUG is not enabled, don't do anything
     */
    if (config_1.CONFIG.Debug === false) {
        return;
    }
    /**
     ...
     */
    if (config_1.CONFIG.Log.exclude.length > 0) {
        for (let excludeMatchString of config_1.CONFIG.Log.exclude) {
            if (typeof msg === "string" &&
                msg.includes(excludeMatchString)) {
                return;
            }
            if (caller_line.includes(excludeMatchString)) {
                return;
            }
        }
    }
