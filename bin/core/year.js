...
class Year {
    constructor(year) {
        this.year = year;
        elog_1.elog("Constructed new year=>" + year);
    }
    getHoliDays() {
        let days = new day_1.DayArray();
        /** New Years */ // 2016-01-01
        days.push(this.weekendNextMonday(new day_1.Day(this.year, 1, 1))); // OK
        ...
        days.push(this.weekendNextMonday(new day_1.Day(this.year, 7, 4))); // OK
        ...
        days.push(this.weekendNextMonday(new day_1.Day(this.year, 12, 25))); // OK
        return days;
    }
    /**
    ...
    getMemorialDay() {
        ...
        let d = this.getDaysInMonth(m);
        for (let y = this.year, date; d > 0; d--) {
        ...
        return new day_1.Day(this.year, m, d);
    }
    /**
     ...
     */
    getDayOfWeekOccurance(month, dayOfWeek, occurance) {
        /**
        ...
        for (let y = this.year, date; d <= maxDays; d++) {
            /** NOTE: javascript native Date type month parameter is zero based */
            date = new Date(y, month - 1, d);
            elog_1.elog(date.getDay() + `=?=${dayOfWeek}`);
            if (date.getDay() == dayOfWeek) {
                break;
            }
            elog_1.elog("no match");
        }
        ...
        return new day_1.Day(this.year, month, d);
    }
    /**
    ...
    weekendNextMonday(day) {
         ...
         * @type {Date}
         */
        elog_1.elog(`constructing new date for weekend check.... ${y}-${m}-${d}`);
        let date = new Date(y, m - 1, d);
        elog_1.elog("day of week is =>" + date.getDay());
        /**
        ...
        if ([0, 6].indexOf(date.getDay()) >= 0) {
            elog_1.elog(`${y}-${m}-${d} is weekend=` + date.getDay());
    ...
    getDaysInMonth(month) {
        ...
        elog_1.elog("lastDay of month " + this.year + "-" + month + "==>" + daysInMonth);
        return daysInMonth;
    }
    /**
    ...
    getGoodFriday() {
        ...
        return new day_1.Day(new Date(easter.getTime() - (86400 * 2 * 1000)));
    }
    /**
